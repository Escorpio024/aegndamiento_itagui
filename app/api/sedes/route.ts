...
export async function POST(req: Request) {
  ...
  const result = await db.prepare('INSERT INTO sedes (nombre, direccion, ciudad, telefono) VALUES (?, ?, ?, ?)')
    .run(nombre.trim(), direccion.trim(), ciudad ?? 'Itagüi', telefono ?? null);

  const sede = await db.prepare('SELECT * FROM sedes WHERE id = ?').get(result.lastInsertRowid);
