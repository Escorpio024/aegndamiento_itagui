...
export async function GET() {
  await runSeed();
  const rows = await db.prepare('SELECT * FROM procedimientos WHERE activo=1 ORDER BY nombre').all();
...
export async function POST(req: Request) {
  ...
  const r = await db.prepare('INSERT INTO procedimientos (cups, nombre, modalidad, contraste) VALUES (?,?,?,?)')
    .run(cups, nombre, modalidad ?? 'IMAGENES_DIAGNOSTICAS', contraste ?? 'Simple');
  return NextResponse.json(
    await db.prepare('SELECT * FROM procedimientos WHERE id=?').get(r.lastInsertRowid),
    { status: 201 }
  );
}
